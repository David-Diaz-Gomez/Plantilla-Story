...
document.getElementById('fontForm').addEventListener('submit', (event) => {
  ...
  // Obtener el valor del campo de texto
  const googleFontsLink = document.getElementById('googleFontsSelect').value;
  ...
  if (isValidGoogleFontsLink(googleFontsLink)) {
    ...
    // Guardar el enlace en el almacenamiento local para recordar la preferencia del usuario
    localStorage.setItem('userFontLink', googleFontsLink);
...
// Verificar si existe una preferencia de fuente guardada en el almacenamiento local
const savedFontLink = localStorage.getItem('userFontLink');
if (savedFontLink) {
  const { fontName, weight } = extractFontNameAndWeight(savedFontLink);
  applyFontWithWeight(fontName, weight);
}